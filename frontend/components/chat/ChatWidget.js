...
const conversationSteps = [
  { key: 'greeting', bot: 'Hello! 👋 I\'m your AI assistant from AI Estate Dubai. Are you looking for a property in Dubai?' },
  ...
  { key: 'confirm', bot: 'Thank you! I\'ve captured all your details. An agent will contact you shortly via WhatsApp or call. Have a great day! 🌟' },
];
...
export function ChatWidget() {
  ...
  const handleSend = (text = inputValue.trim()) => {
    ...
    if (nextStepIndex < conversationSteps.length) {
      ...
      triggerBotMessage(nextStep.bot);
  ...
  const sendLeadToBackend = async (data) => {
    try {
      await axios.post(
        ...
        { ...data, agentId: 'demo-agent-123' } // In real app, get from script param
      );

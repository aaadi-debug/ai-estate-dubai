...
export const signup = async (req, res) => {
  try {
    ...
    const token = jwt.sign({ agentId: agent._id, role: agent.role }, JWT_SECRET, { expiresIn: '30d' });
...
export const login = async (req, res) => {
  try {
    ...
    console.log('Login attempt:', { email }); // ← Add this for debugging
    ...
    res.json({
      ...
      agent: {
        ...
        role: agent.role, // Make sure role is always included
      },
      token,
    });

    console.log('Login Done For:', { email }); // ← Add this for debugging
